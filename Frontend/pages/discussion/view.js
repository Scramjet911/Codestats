...
class ViewDiscussion extends Component{
    ...
    addComment = ()=>{
        ...
        if(this.state.isLoggedIn && localval){
            localval = JSON.parse(localval);
            // console.log(localval);
            fetch(discussurl + `comment/${localval.user._id}/${this.state.discuss._id}`,{
                method:"POST",
                headers:{
                ...
                body:JSON.stringify({body:this.commentArea.current.value})
            })
            .then(data=>{
                if(data.status!==200){
            ...
            <div>
                ...
                <div id="row">
                    <div className={styles.discussion_body}>
                        <div className={styles.date}>{moment(this.state.discuss.createdAt).format('MMMM Do YYYY')}</div>
                        <div className="title">{this.state.discuss.title}</div>
                        <div className={styles.details}>
                            {this.state.discuss.category.map((cat,index)=>
                                <div key={index} className="tags-discuss">{cat}</div>
                            )}
                            <div className={styles.username}>{this.state.discuss.authorname}</div>
                        </div>
                        ...
                        {!this.state.isLoggedIn && (
                            <div className={styles.control}>Login To Comment</div>
                        )}
                        Comments:
                        {this.state.discuss.comments.map((com,index)=>{
                            return(
                                <div key={index} id="row">
                                    <div className={styles.username}>{com.author}</div>
                                    <div className={styles.reply}>{com.body}</div>
                                </div>    
                            );
                        })}
                        </div>
